...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
  	;PUSH {R4 - R11} 				; save callee registers R4-R11 not used
	
readSSI01    
	LDR R1, =SSI0_SR_R				; read SSI0_SR_R
	LDR R2, [R1]
	AND R2, #0x10					; check if bit 4 is high
	CMP R2, #0x00					
	BNE readSSI01					; loop back if high
	
	LDR R1, =GPIO_PORTA_DATA_R		; clear D/C=PA6
	LDR R2, [R1]
	BIC R2, #0x40
	STR R2, [R1]
	LDR R1, =SSI0_DR_R				; write command to SSI0_DR_R
	STR R0, [R1]
...
readSSI02	
	...
	AND R2, #0x10					; check if bit 4 is high
	CMP R2, #0x00					
	BNE readSSI02					; loop back if high
	
    ;POP {R4 - R11}					; pop callee registers R4-R11 not used
    BX  LR                          ; return
...
readSSI03
	...
	CMP R2, #0x00					
	BEQ readSSI03					; loop back if low
	
	LDR R1, =GPIO_PORTA_DATA_R		; set D/C = PA6 to one
	LDR R2, [R1]
	ORR R2, #0x40
	STR R2, [R1]
	
	LDR R1, =SSI0_DR_R				; write data to SSI0_DR_R
	STR R0, [R1]
