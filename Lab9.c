...
uint32_t TxCounter = 0;				// transmission counter
uint32_t state = 0;						// state of mailbox
uint32_t adc = 0;							// holds data from ADC
uint8_t out[8];
char packet;										// sent/received data

void SysTick_Init(void){
	NVIC_ST_CTRL_R = 0;					// disable SysTick for init
	NVIC_ST_RELOAD_R = 1333334;	// 60 Hz or 16.667 ms on 80MHz clock
...
uint32_t Convert(uint32_t input){
	uint32_t result = 0; 								// conversion result
...
int main(void){ 
  ...
  ST7735_InitR(INITR_REDTAB);	// initalize LCD
  ...
  LCD_OutFix(0);
  ST7735_OutString(" cm");						// prints "0.000 cm" by default
